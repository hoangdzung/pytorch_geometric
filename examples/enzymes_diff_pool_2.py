...
class Net(torch.nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        x = self.lin2(x)
        return F.log_softmax(x, dim=-1), l1 + l2, e1 + e2, s1 + s2, a1 + a2
...
def train(epoch):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        output, link_loss, ent_loss, s_loss, adj_loss = model(data.x, data.adj, data.mask)
        loss = F.nll_loss(output, data.y.view(-1)) + int(args.l) * link_loss + int(args.e) * ent_loss + int(args.s) * s_loss + int(args.a) * adj_loss
        loss.backward()
